...
## [Unreleased]
- **H1 RAFT 训练与容量边界验证 (2026-08-14)** — H1 PLE 同流程 RAFT
  - **训练**: `email_raft_h1ple_256_ple.pth` (同 H2 流程: 6000条×2ep, lr 5e-5, from H1 PLE SFT), loss **0.0208** (vs H2 RAFT 0.014)
  - **验证 (证据注入问五通一服务)**: H1 RAFT **召回 4/5 通**(银保/经代/电销/团险, 缺网销) 后陷入重复循环; 提高惩罚 (rp=1.5) 不解决反而崩坏 → **容量瓶颈非解码问题**
  - **结论**: RAFT 证据利用行为在 H1 也已习得 (输出从证据复述起步), 但 6.66M 核心容量撑不住 5 项枚举完整复述; **精确问答场景推荐 H2 RAFT** (`email_raft_h2ple_384_ple.pth`, 24.95M 完美 5/5), H1 适合 3 项以内短枚举/单事实
  - 排查记录: 初次验证输出幻觉 (党支部) 为 argmax+低惩罚不稳定, 严格单变量重测后确认证据利用有效; teacher-forcing 诊断 + chat template 核查排除数据/加载问题
