...
def train_epoch(epoch, loader, iters, ref_model, lm_config, start_step=0, wandb=None, beta=0.1):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            model.eval()
            suffix = '_moe' if lm_config.use_moe else ('_ple' if lm_config.use_ple else '')
            ckp = f'{args.save_dir}/{args.save_weight}_{lm_config.hidden_size}{suffix}.pth'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_ple', default=0, type=int, choices=[0, 1], help="是否使用PLE每层嵌入架构（0=否，1=是，参考esp32-ai）")
    parser.add_argument('--ple_dim', default=None, type=int, help="PLE每层嵌入维度（默认 hidden_size//4）")
    ...
    if args.use_ple:
        lm_config.use_ple = True
        if args.ple_dim: lm_config.ple_dim = args.ple_dim
