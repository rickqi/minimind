...
# 四方案对比: H2 RAG+RAFT / H3混合 / H3混合+RAFT / H3混合+RAFT+RAG
models = {
    'H2 RAG+RAFT': (384, 8, 128, 'full_sft_h2_raft'),
    'H3混合': (512, 8, 128, 'full_sft_h3_mixed'),
    'H3混合+RAFT': (512, 8, 128, 'full_sft_h3_mixed_raft'),
}
loaded = {k: load(*v) for k, v in models.items()}

def gen(model, messages, max_new=120):
