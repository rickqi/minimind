...
# ========== RAG 检索 (jieba IDF, 与 rag_medical.py 一致) ==========
...
# ========== 模型加载 ==========
def load_model(hidden, layers, ple, weight):
...
models = {
    'H1(RAFT)': load_model(256, 6, 96, 'full_sft_h1_raft'),
    'H2(RAFT)': load_model(384, 8, 128, 'full_sft_h2_raft'),
}
...
for q in questions:
    ...
    print('[证据]', ev[:80].replace('\n',' | '))
    for name, m in models.items():
        a_no = gen(m, make_messages(q, None))
        a_rag = gen(m, make_messages(q, ev))
        print(f'\n  [{name} 无RAG] {a_no[:110]}')
        print(f'  [{name} RAG  ] {a_rag[:110]}')
PYEOF
