...
- [x] **A 阶段全量附件增强**: 1,690,017 样本 (3.5GB), 历史基线 ~5K → **300x 提升**; 表格 88K (5.2%) + 长文档 39K (2.3%)
- [x] **分月运行脚本** (run_pretrain_by_month.py): 规避全量内存不足 (2GB vs 20GB, D 状态阻塞)
- [ ] 全量管线重跑 (B2 QA / C RAFT 附件增强, 需 LLM API key — 当前 .env 未配置)
- [ ] 训练验证 (附件增强预训练 → 模型)
