...
| 训练 (2000 对×2ep) | loss 0.55-0.69 (硬负样本, 健康波动) |
| 分类回归 | 50% (与 SFT 相同, **无退化**) |
| 附件问答 | DPO 开始倾向"引用附件"回复 (vs SFT 重复内容) |
| 关键发现 | thread_id 带 `_数字` 后缀需剥离; 长附件样本训练慢 (AMD 890M) |

**结论**: B 方案核心完成 — 附件富矿 (51K md) 已注入 DPO 数据 (68.5% 覆盖), 训练超参已调优 (lr 1e-6/beta 0.3), 附件问答行为有正向变化。H1 小模型限制下差异有限, 建议 H2 全量增强集长训练进一步验证。
