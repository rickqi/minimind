...
本章逐行拆解 `MiniMindConfig`(`MiniMindConfig — model_minimind.py:~10-50 (@67f114a)`)的每一个超参数,解释为什么是 `hidden_size=768`、`intermediate_size=2432`、`rope_theta=1e6`,并用代码验证 64M 参数量的精确来源。

## 学习目标

- 说出 `MiniMindConfig` 的 18 个核心字段及其取值理由
...
| 文件 | 用途 |
|---|---|
| [`ch03.ipynb`](./ch03.ipynb) | 主 notebook(逐行拆解 18 个超参数 + 参数量计算) |
...
- `MiniMindConfig — model_minimind.py:~10-50 (@67f114a)`(`MiniMindConfig` 类,全部超参数定义)
